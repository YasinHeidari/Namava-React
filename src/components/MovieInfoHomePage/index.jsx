...
import SpinnerLoading from '../Loading/SpinnerLoading'; // Import the SpinnerLoading component
import "./index.css";
    ...
    if (movie && movie.id) {
      ...
      fetch(`https://api.themoviedb.org/3/movie/${movie.id}/credits?api_key=${apiKey}`)
        ...
        .then(data => {
          ...
          // Delay setting loading to false to ensure the spinner shows for at least 3 seconds
          setTimeout(() => {
            setLoading(false);
          }, 3000);
        })
      ...
      // Clear the spinner timeout when the component unmounts or when movie changes
  ...
  return (
    <div className="containerMovieInfo position-relative" style={{ backgroundImage: `url(https://media.themoviedb.org/t/p/original/${movie.backdrop_path})`, backgroundSize: 'cover', backgroundPosition: 'left top' }}>
      ...
      <div className="container h-100">
        {showSpinner ? ( // Conditionally render the SpinnerLoading component if showSpinner is true
          <SpinnerLoading />
        ) : (
          expanded && (
                  ...
                  <Link to={`/movie/${movie.id}`} className="movieInfoInnerMore d-flex flex-row align-center gap-1 border-radius-12 line-height-42">
                    <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 30 30" fill="#fff">
                      <g id="Group_2349" data-name="Group 2349" transform="translate(2641 -16124)">
                        ...
                        <path id="Path_5398" data-name="Path 5398" d="M1530.066-359.976a12.023,12.023,0,0,0-8.558-3.545,12.022,12.022,0,0,0-8.558,3.545,12.024,12.024,0,0,0-3.545,8.558,12.024,12.024,0,0,0,3.545,8.558,12.022,12.022,0,0,0,8.558,3.545,12.023,12.023,0,0,0,8.558-3.545A12.117,12.117,0,0,0,1530.066-359.976Zm-8.558,17a1.57,1.57,0,0,1-1.57-1.57,1.57,1.57,0,0,1,1.57-1.57,1.57,1.57,0,0,1,1.57,1.57A1.57,1.57,0,0,1,1521.508-342.98Zm1.57-7.109a1.57,1.57,0,0,1-1.57,1.57,1.57,1.57,0,0,1-1.57-1.57v-7.493a1.57,1.57,0,0,1,1.57-1.57,1.57,1.57,0,0,1,1.57,1.57Z" transform="translate(-1104.395 15787.685) rotate(180)"></path>
                      </g>
                    </svg>
