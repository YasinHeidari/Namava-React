    ...
    return (
        ...
        <div className="d-flex flex-column align-start gap-3">
            ...
            <Dropdown overlay={<Menu onClick={handleMenuClick} items={menuItems} />} trigger={['click']} placement='bottom'>
                ...
                <div className='d-flex justify-start align-center gap-2 seasonCounts border-radius-12'>
                    <div>
                        <span className='white-color font-14 font-weight-normal'>تعداد کل فصل‌ها: {numberOfSeasons}</span>
                        <span className='white-color font-14 font-weight-normal'>وضعیت پخش: {status === 'Ended' ? 'پایان انتشار سریال': 'در حال پخش'}</span>
                    </div>
            ...
            <div className='d-lg-none d-flex flex-column gap-2 col-12 ' >
                {episodes.map((episode) => {
                    if (!episode.still_path) {
                        return null;
                    }
                    return (
                        <Link to="/" key={episode.id} className='d-flex flex-column gap-1 col-12 ' >
