    ...
    useEffect(() => {
        const fetchHeroData = async () => {
            try {
                ...
                const moviesWithDetails = await Promise.all(upcomingData.results.map(async (movie) => {
                    const movieResponse = await fetch(`https://api.themoviedb.org/3/movie/${movie.id}?api_key=${apiKey}&language=en-US&append_to_response=images,credits&include_image_language=en,jp,null`);
                    const movieData = await movieResponse.json();
                    const logo = movieData.images?.logos[0]?.file_path;
                    return { ...movie, credits: movieData.credits, logoUrl: logo };
                }));
                                    ...
                                    <div className="d-flex flex-column justify-btw align-start h-100">
                                        <div className="col-4 d-flex flex-column justify-center align-start align-self-start gap-4 container-padding-2 h-100">
                                            <Link to={`/movie/${movie.id}`}>
                                                {movie.logoUrl && <img loading="lazy" src={`https://image.tmdb.org/t/p/w300/${movie.logoUrl}`} alt={movie.name || movie.title} />}
                                            </Link>
